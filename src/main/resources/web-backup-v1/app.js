const stations = ["KÖLN", "MAINZ", "BONN"];
const palette = { KÖLN: "#35d7ff", MAINZ: "#4d7cff", BONN: "#40e0a1" };
...
function updateCard(station, reading) {
    ...
    const domId = station === "KÖLN" ? "KOELN" : station;

    document.getElementById(`value-${domId}`).textContent =
        Number(reading.value).toFixed(0);

    document.getElementById(`trend-${domId}`).textContent =
        trendSymbol(reading.trend);

    document.getElementById(`time-${domId}`).textContent =
        fmtTime(reading.timestamp);

    const percent = Math.max(4, Math.min(100, Number(reading.value) / 6));

    document.getElementById(`water-${domId}`).style.width =
        `${percent}%`;
